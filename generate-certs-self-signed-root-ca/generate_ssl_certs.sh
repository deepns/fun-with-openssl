...
generate_root_ca()
{
    ...
    openssl req -x509 -nodes -sha256 -newkey rsa:2048 -days 1024 -out $ROOT_CA.crt -subj "/C=US/ST=DE/O=MyCert, Inc./CN=mycert.com" -keyout $ROOT_CA.key
}
...
generate_cert_signed_by_root_ca()
{
    ...
    openssl req -new -nodes -newkey rsa:2048 -subj "/C=US/ST=DE/O=ExampleOrg, Inc./CN=127.0.0.1" -out $NODE.csr -keyout $NODE.key
